...
impl TemplateSource
{
    pub fn new<P>(prefix: P, suffix: P) -> TemplateSource
    ...
    pub fn load(&self, reg: &mut Handlebars) -> Result<(), Box<Error>>
    {
        ...
        println!("Loading templates from path {:?}", self.prefix);
...
//            debug!("register template {:?}", tpl_name);
            reg.register_template_file(&tpl_canonical_name, &tpl_path)?;
        }
        Ok(())
    }
}
